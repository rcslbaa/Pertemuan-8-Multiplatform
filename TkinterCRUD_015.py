...
def submit():
    try:
        ...
        messagebox.showerror("Error", f"Kesalahan: {0}")
...
def update():
    try:
        ...
        if not record_id:
            raise Exception("Pilih record yang akan diupdate.")
...
def delete():
    try:
        record_id = selected_record_id.get()
        if not record_id:
            raise Exception("Pilih record yang akan dihapus.")
...
#Menyesuaikan posisi teks di setiap kolom tengah
